...
```jsonc
{
  "id": "CLF-D3-0142",                    // Unique within the cert. Format: <CERT>-D<DOMAIN>-<NUMBER>
...
The dashboard cert toggle, mock exam, domain practice, history, and stats pages all pick up the new cert automatically once the cert config, question files, and loaders are wired. The cert toggle on the guest dashboard is hidden while only one cert has `status: 'active'` and re-appears automatically once a second cert is flipped to active. No frontend or backend code changes are needed regardless of how many domains the cert has; the `exam_attempts.domain_scores` JSONB column scales per-cert.
...
- **Conventional commits** in commit messages: `feat:`, `fix:`, `chore:`, `docs:`, `refactor:`, `style:`, `test:`.
  - Good: `fix(clf-c02): correct answer for D3-0142`
...
|---|---|---|
| AWS orange (primary) | `aws-orange`, `aws-orange-end` | Primary CTAs, active state, brand accents |
...
**Domain pills, scores, and badges** use the `aws-orange` Tailwind token (`text-aws-orange`, `bg-aws-orange/20`, etc.) defined in `tailwind.config.js`. The token automatically themes between light (`#EA8C00`) and dark (`#FF9900`) modes. Do not introduce per-domain colours or hand-pick hex values.

**Spacing, radius, shadow**

- Card padding: `p-4 md:p-6` (small) or `p-6 md:p-8` (large).
- Card radius: `rounded-lg`. Buttons: `rounded-lg`. Pills: `rounded-lg` or `rounded-full`.
