...
// landing on `/aws` get the canonical home immediately. The rule is FORCED
// (`301!`) so it takes precedence over the static `/aws/index.html` that
// Astro emits from `src/pages/aws/index.astro` (a non-forced rule would be
// shadowed by that file, leaving a soft meta-refresh instead of a real 301).
// When a second provider ships, this section becomes empty (multi-provider
// gets the real hub back). The bare path matches `/aws` exactly; cert-scoped
// routes like `/aws/clf-c02` are not affected and fall through.
